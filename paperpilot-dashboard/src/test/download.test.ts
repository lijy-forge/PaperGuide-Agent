...
describe("artifact downloads", () => {
  ...
  it("uses a safe fallback for missing filename", () => expect(safeDownloadFilename(undefined, "abcdefghi")).toBe("paperpilot-report-abcdefgh.md"));
  it("rejects unsafe extensions", () => expect(safeDownloadFilename('attachment; filename="report.exe"', "abcdefghi")).toBe("paperpilot-report-abcdefgh.md"));
  it("uses PDF MIME when a stale header says markdown", () => expect(safeDownloadFilename('attachment; filename="report.md"', "abcdefghi", "application/pdf")).toBe("paperpilot-report-abcdefgh.pdf"));
  it("uses HTML MIME for a missing filename", () => expect(safeDownloadFilename(undefined, "abcdefghi", "text/html; charset=utf-8")).toBe("paperpilot-report-abcdefgh.html"));
