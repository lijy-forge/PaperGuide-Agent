...
describe("recent task history", () => {
  ...
  it("recovers from damaged JSON", () => { localStorage.setItem("paperguide.recentTasks", "not json"); expect(getRecentTasks()).toEqual([]); });
  it("filters malformed records without rejecting valid history", () => { localStorage.setItem("paperguide.recentTasks", JSON.stringify([task(1), { taskId: 42, localTitle: "bad" }])); expect(getRecentTasks().map((item) => item.taskId)).toEqual(["task-1"]); });
  it("reads the legacy key and migrates on the next write", () => { localStorage.setItem("paperguide.recentTasks.v1", JSON.stringify([task(1)])); expect(getRecentTasks()).toHaveLength(1); addRecentTask(task(2)); expect(localStorage.getItem("paperguide.recentTasks")).toContain("task-2"); expect(localStorage.getItem("paperguide.recentTasks.v1")).toBeNull(); });
