...
export function researchSubmissionError(error: unknown): SubmissionError {
  if (error instanceof ApiError) {
    ...
    if (error.code === "API_UNAVAILABLE" || error.status === undefined) {
      return { message: "PaperGuide API 不可用。", hostOffline: false };
    }
  }
...
export function researchSubmissionErrorMessage(error: unknown): string {
  ...
  return result.hostOffline
    ? `${result.message} 请运行：paperguide server start`
    : result.message;
}
    ...
    >
      {error && (
        <Alert
          ...
          description={error.hostOffline ? <span>请运行：<Typography.Text code>paperguide server start</Typography.Text></span> : undefined}
