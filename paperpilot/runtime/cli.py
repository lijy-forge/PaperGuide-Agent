"""Standard-library command-line entry point for PaperPilot research runs."""
...
from paperpilot.application import ResearchRequest, ResearchTask, ResearchTaskStatus
...
from paperpilot.execution import TaskExecutorProtocol
...
LOGGER = logging.getLogger("paperpilot.runtime")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    demo = commands.add_parser("demo", help="run the offline PaperPilot demo")
    demo.add_argument(
        "--question",
        default=None,
    ...
    try:
        ...
        if arguments.command == "server":
            if settings.mode == "demo" and task_host_factory is TaskHost:
                from paperpilot.demo import create_demo_application
        ...
        if demo_command:
            from paperpilot.demo import DEMO_QUESTION
        ...
        _write_json(
            ...
            {
                "status": "failed",
                "error": "task host is unavailable; run 'paperpilot server start'",
            },
        )
        return 1
