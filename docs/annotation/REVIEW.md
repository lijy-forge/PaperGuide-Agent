...
## 二、动态环境下的视觉SLAM与语义建图（已完成复核）

**问题**：动态环境下的视觉SLAM与语义建图研究进展

复核结果已应用：移除 6 条判定有误的、移除 1 条无法判断的、新增 9 条，
标注集由 15 条变为 17 条。此节保留供追溯。

**我在这里犯的是方法错误而非判断失误**：筛选代码写的是命中关键词后取前 15 条
（`[:15]`），按 bib 文件顺序截断而不是按相关度挑选。结果是排在后面、同时命中
多个关键词的高相关论文被位置截掉，而排在前面、只命中一个关键词的论文被留下。
